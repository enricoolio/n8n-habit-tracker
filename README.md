...
```
You (Telegram) --> n8n Workflows --> Claude API (food analysis)
                                 --> Supabase (food logs, analytics)
                                 --> Notion (habit checkboxes)

iOS Shortcuts --> n8n Webhook --> Supabase (step count)

Cron (9pm) --> n8n --> Telegram (evening check-in)
Cron (Sun) --> n8n --> Claude (weekly recap) --> Telegram
```
...
|---|---------|---------|---------|
| 01 | `telegram-food-logger` | Telegram message | Log food via text/photo/voice, get macro breakdown |
| 02 | `evening-checkin` | Cron (9pm daily) | Ask about habits, update Notion + Supabase |
...
3. In each workflow, update the credentials:
   ...
   - **HTTP Header Auth** (for Claude): Header name = `x-api-key`, value = your Anthropic API key
4. In workflow `02-evening-checkin.json`, replace `YOUR_TELEGRAM_CHAT_ID` with your actual chat ID
5. Activate all workflows

### Step 6: iOS Shortcut for Steps
...
### Step 7: Add Your Supplement Plan
...
```
Throughout the day:
  Send food photos/text to Telegram bot --> instant macro feedback
...
9:00 PM: Bot asks "what did you do today?"
  ...
  Bot: logs everything, updates Notion, gives daily score
